...
import { Badge } from "@pkgs/ui/components/badge";
import {
  Command,
  CommandGroup,
  CommandItem,
...
interface MultipleSelectorProps {
  ...
  onMaxSelected?: (v: string, maxLimit: number) => void;
...
>(
  (
        ...
        if (input) {
          if (
            ...
            if (!lastSelectOption?.fixed) {
              handleUnselect(selected.at(-1) as Option);
            }
          }
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: no need to add eventHandler
    useEffect(() => {
      if (open) {
    ...
    useEffect(() => {
      ...
      exec();
      // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [onSearch, debouncedSearchTerm, groupBy, open, triggerSearchOnFocus]);
          ...
          <CommandItem
            ...
            onSelect={(value: string) => {
              if (selected.length >= maxSelected) {
                onMaxSelected?.(value, selected.length);
                return;
              }
              setInputValue("");
                      ...
                      {dropdowns.map((option) => {
                        return (
                          <CommandItem
                            ...
                            onSelect={(v) => {
                              if (selected.length >= maxSelected) {
                                onMaxSelected?.(v, selected.length);
                                return;
                              }
                              setInputValue("");
